#!/usr/bin/env bash
#
# SPIGA Stage 2: Fine-tuning with Landmarks + Pose
# Distributed training with DDP and SLURM
#
# Submit with: sbatch submit_stage2.sh
# Requires: Stage 1 checkpoint (checkpoints/checkpoint_stage1/best_model.pth)

#SBATCH --job-name=spiga_stage2
...
#SBATCH --output=spiga_stage2.out
...
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-spiga_stage2_${SLURM_JOB_ID}}"
...
LOG_DIR="./logs"
STAGE1_MODEL="${CHECKPOINT_DIR}/checkpoint_stage1/best_model.pth"

mkdir -p "${LOG_DIR}"

# Verify Stage 1 checkpoint exists
if [ ! -f "${STAGE1_MODEL}" ]; then
    echo "ERROR: Stage 1 checkpoint not found at ${STAGE1_MODEL}"
    echo "Please run Stage 1 training first!"
    exit 1
fi

echo "=========================================="
echo "SPIGA Stage 2: Fine-tuning"
echo "=========================================="
