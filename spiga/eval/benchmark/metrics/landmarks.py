...
class MetricsLandmarks(Metrics):
    ...
    def compute_error(self, data_anns, data_pred, database, select_ids=None):
        ...
        for img_id, anns in enumerate(data_anns):
            ...
            if self.database in ["merlrav"]:
                ...
                self.error["nme_per_img"].append(
                    np.sum(self.error["ne_per_img"][img_id]) / 68
                )
            else:
                self.error["nme_per_img"].append(
                    np.mean(self.error["ne_per_img"][img_id])
                )

        # Cumulative NME
        self.error["cumulative_nme"] = self._cumulative_error(
            self.error["nme_per_img"], bins=self.bins
        )
    ...
    def metrics(self):
        ...
        if self.data_type == "test" and len(subsets) > 0:
            ...
            for subset, img_filter in subsets.items():
                ...
                for percent_id, percentile in enumerate(self.percentile):
                    print("NME_P%i: %.3f" % (percentile, nmpe[percent_id]))
                    self.metrics_log["subset"][subset]["nme_p%i" % percentile] = nmpe[
                        percent_id
                    ]
                print("AUC_%i: %.3f" % (self.nme_thr, auc))
        ...
        for k, v in self.error["ne_per_ldm"].items():
            ...
            for percentile in self.percentile:
                self.metrics_log["npe%i_per_ldm" % percentile][k] = np.percentile(
                    v, percentile
                )
    ...
    def _update_lnd_param(self):
        ...
        if os.path.exists(db_info_file):
            ...
            # Setup eval based on dataset without user interaction
            dataset = os.environ["DATASET"]
            # Dataset-specific normalization setup
            if dataset == "300wpublic":
                print("Using 300W Public landmarks configuration.")
                self.nme_norm = "corners"
                self.nme_thr = 8
            elif dataset == "300wprivate":
                print("Using 300W Private landmarks configuration.")
                self.nme_norm = "lnd_bbox"
                self.nme_thr = 7
            elif dataset == "wflw":
                print("Using WFLW landmarks configuration.")
                self.nme_norm = "corners"
                self.nme_thr = 10
            elif dataset == "cofw68":
                print("Using COFW-68 landmarks configuration.")
                self.nme_norm = "lnd_bbox"
                self.nme_thr = 7
            elif dataset == "merlrav":
                print("Using MERLRAV landmarks configuration.")
                self.nme_norm = "lnd_bbox"
                self.nme_thr = 7
            else:
                print(
                    f"[Warning] Unknown dataset {dataset}, using default normalization."
                )
                self.nme_norm = "bbox"
                self.nme_thr = 0.08
            ...
            raise ValueError(
                "Database %s specifics not defined. Missing db_info.json"
                % self.database
            )
    ...
    def _auc_fr_metrics(self, cumulative, base):
        if not base:
            ...
            auc = (simpson(cumulative, x=base) / self.nme_thr) * 100.0
