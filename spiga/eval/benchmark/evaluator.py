...
def main():
    ...
    pars.add_argument(
        ...
        nargs="+",
        type=str,
        help="Absolute path to the prediction json file (Multi file)",
    )
    pars.add_argument(
        "--eval",
        nargs="+",
    ...
    pars.add_argument(
        "-w", "--log_wandb", action="store_true", help="Log results to wandb"
    )
    args = pars.parse_args()

    for pred_file in args.pred_file:
        benchmark = get_evaluator(
            pred_file, args.eval, args.save, log_wandb=args.log_wandb
        )
        benchmark.metrics()
...
class Evaluator:
    def __init__(
        self, data_file, evals=(), save=True, process_err=True, log_wandb=None
    ):
        # Inputs
        ...
        # Setup logs to wandb, get project name from env var
        print(f"Checking wandb var: {log_wandb}")
    ...
    def compute_error(self, anns, pred, select_ids=None):
        ...
        for eval in self.evals:
            self.error[eval.name] = eval.compute_error(
                anns, pred, database_ref, select_ids
            )
    ...
    def metrics(self):
        for eval in self.evals:
            ...
            # Log to wandb
            if self.log_wandb:
                wandb.log({eval.name: self.metrics_log[eval.name]})

        if self.save:
            file_name = self.data_dir + "/metrics_%s_%s.txt" % (
                self.database,
                self.data_type,
            )
    ...
    def _dict2text(self, name, dictionary, num_tab=1):
        ...
        for k, v in dictionary.items():
            if isinstance(v, OrderedDict) or isinstance(v, dict):
                text += "{}{}".format(
                    prev_tabs, self._dict2text(k, v, num_tab=num_tab + 1)
                )
            else:
...
def get_evaluator(
    pred_file, evaluate=("lnd", "pose"), save=False, process_err=True, log_wandb=None
):
    eval_list = []
    if "lnd" in evaluate:
